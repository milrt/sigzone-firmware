...
static int cmd_status(const struct shell *shell, size_t argc, char **argv)
{
    ...
    if (status == PSA_SUCCESS) {
        ...
        shell_error(shell, "Error: %d", status);
        return -EIO;
    }
    return 0;
}
...
{
    if (argc != 2) {
        shell_error(shell, "Usage: create <pin>");
        return -EINVAL;
    }
    psa_status_t status = bitcoin_client_create(argv[1]);
    if (status != PSA_SUCCESS) {
        shell_error(shell, "Create failed: %d", status);
        return -EIO;
    }
    shell_print(shell, "Wallet created");
    return 0;
}
...
{
    if (argc != 3) {
        shell_error(shell, "Usage: recover <pin> <mnemonic>");
        return -EINVAL;
    }
    psa_status_t status = bitcoin_client_recover(argv[1], argv[2]);
    if (status != PSA_SUCCESS) {
        shell_error(shell, "Recover failed: %d", status);
        return -EIO;
    }
    shell_print(shell, "Wallet recovered");
    return 0;
}
...
static int cmd_destroy(const struct shell *shell, size_t argc, char **argv)
{
    ...
    if (status != PSA_SUCCESS) {
        shell_error(shell, "Destroy failed: %d", status);
        return -EIO;
    }
...
static int cmd_open(const struct shell *shell, size_t argc, char **argv)
{
    ...
    if (status != PSA_SUCCESS) {
        shell_error(shell, "Open failed: %d", status);
        return -EIO;
    }
...
static int cmd_close(const struct shell *shell, size_t argc, char **argv)
{
    ...
    if (status != PSA_SUCCESS) {
        shell_error(shell, "Close failed: %d", status);
        return -EIO;
    }
...
static int cmd_get_pubkey(const struct shell *shell, size_t argc, char **argv)
{
    ...
    if (status != PSA_SUCCESS) {
        shell_error(shell, "Get pubkey failed: %d", status);
        return -EIO;
    }
    ...
    if (status != PSA_SUCCESS) {
        shell_error(shell, "Script generation failed: %d", status);
        return -EIO;
    }
    ...
    if (status == PSA_SUCCESS) {
        ...
        shell_error(shell, "Address generation failed: %d", status);
    }
...
static int cmd_sign_hash(const struct shell *shell, size_t argc, char **argv)
{
    ...
    if (status != PSA_SUCCESS) {
        shell_error(shell, "Sign failed: %d", status);
        return -EIO;
    }
...
SHELL_STATIC_SUBCMD_SET_CREATE(
    ...
    SHELL_CMD_ARG(create, NULL, "Create new wallet <pin>", cmd_create, 2, 0),
    SHELL_CMD_ARG(recover, NULL, "Recover wallet <pin> <mnemonic>", cmd_recover, 3, 0),
